...
class YearlyProgressSubscriptionManager(private val context: Context) {
  ...
  val shouldShowAds: LiveData<SubscriptionStatus>
    get() = _shouldShowAds
  ...
  suspend fun processPurchases() {
    ...
    val productDetailsResult =
        withContext(Dispatchers.IO) { billingClient.queryProductDetails(params.build()) }
    ...
    if (productDetailsList.isNullOrEmpty()) {
      withContext(Dispatchers.Main) {
        Toast.makeText(
                context,
                context.getString(R.string.not_available_in_your_region),
                Toast.LENGTH_LONG)
            .show()
      }
  ...
  fun redirectToSubscriptionPage() {
    ...
    val intent =
        Intent(
            Intent.ACTION_VIEW,
            Uri.parse(
                "https://play.google.com/store/account/subscriptions?sku=${AD_FREE_PRODUCT_ID}&package=${packageName}"))
