...
fun getTodayOrNearestEvent(contentResolver: ContentResolver): Event? {
    ...
    val selectionToday = "${CalendarContract.Events.DTSTART} >= ? AND ${CalendarContract.Events.DTSTART} < ?"
    val selectionArgsToday = arrayOf(startOfDay.toString(), endOfDay.toString())
    val sortOrder = "${CalendarContract.Events.DTSTART} ASC LIMIT 1"

    contentResolver.query(uri, projection, selectionToday, selectionArgsToday, sortOrder)?.use { cursor ->
        if (cursor.moveToFirst()) {
            ...
            val startTimeUtc = cursor.getLong(cursor.getColumnIndexOrThrow(CalendarContract.Events.DTSTART))
    ...
    contentResolver.query(uri, projection, selectionUpcoming, selectionArgsUpcoming, sortOrder)?.use { cursor ->
        if (cursor.moveToFirst()) {
            val id = cursor.getLong(cursor.getColumnIndexOrThrow(CalendarContract.Events._ID))
            val title = cursor.getString(cursor.getColumnIndexOrThrow(CalendarContract.Events.TITLE))
            val description = cursor.getString(cursor.getColumnIndexOrThrow(CalendarContract.Events.DESCRIPTION))
