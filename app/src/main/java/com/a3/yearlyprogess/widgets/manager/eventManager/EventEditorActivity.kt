...
class EventEditorActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val event: Event? =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
          ...
          @Suppress("DEPRECATION") intent.getParcelableExtra("event")
        }
  ...
  private fun setUpDateTimePickers() {
    ...
    binding.editTextStartDate.setOnClickListener {
      datePicker
          .setTitleText("Choose event first day")
    ...
    binding.editTextStartTime.setOnClickListener {
      timePicker
          .setTitleText("Choose event starting time")
    ...
    binding.editTextEndDate.setOnClickListener {
      datePicker
          .setTitleText("Choose event last day")
    ...
    binding.editTextEndTime.setOnClickListener {
      timePicker
          .setTitleText("Choose event ending time")
      ...
      when (it.itemId) {
        R.id.save_config -> {
          ...
          if (!isAddMode) {
            ...
            val appWidgetIds =
                appWidgetManager.getAppWidgetIds(ComponentName(this, EventWidget::class.java))
  ...
  private fun getHourMinuteLocal(time: Long): String {
    return if (is24HourFormat(this)) SimpleDateFormat("HH:mm", Locale.getDefault()).format(time)
