...
class ImportEventAdapter(
  ...
  override fun onBindViewHolder(holder: ImportEventsViewHolder, position: Int) {
    ...
    holder.binding.eventCheck.isChecked = tracker?.isSelected(position.toLong()) == true
    ...
    holder.binding.eventCheck.setOnClickListener { handleSelection(holder, position) }
...
class ImportEventItemDetailsLookup(private val recyclerView: RecyclerView) :
  ...
  override fun getItemDetails(event: MotionEvent): ItemDetails<Long>? {
    ...
    if (view != null) {
      return (recyclerView.getChildViewHolder(view) as ImportEventsViewHolder).getItemDetails()
    }
    return null
  }
}
